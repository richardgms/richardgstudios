...
const StudioModuleIcon = dynamic(
  ...
  { ssr: false, loading: () => <Banana className="w-4 h-4 md:w-5 md:h-5 text-white" /> }
);
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0 md:flex md:flex-col md:flex-1 md:mt-3">
          <h2 className="font-display font-bold text-sm md:text-[17px] text-text-primary leading-tight truncate md:whitespace-normal">
            {mod.name}
          </h2>
        ...
        `}>
          {mod.stats.map((stat) => (
            <div key={stat.label} className="text-right md:text-left">
              <p className={`text-[11px] md:text-xs font-semibold font-mono leading-none ${mod.accentColor}`}>
                {stat.value}
              </p>
...
export default function HubPage() {
  return (
    <div className="flex-1 overflow-y-auto flex flex-col items-center px-4 md:px-8 relative">
      ...
      <div className="w-full max-w-3xl flex-1 flex flex-col justify-center gap-6 md:gap-16 py-8 md:py-16 pb-24 md:pb-16 relative z-10">
        ...
        >
          ...
          <div className="flex justify-center mb-4 md:mb-6">
            <div className="w-10 h-10 md:w-14 md:h-14 rounded-2xl overflow-hidden">
              <Image src="/icon.png" alt="Richard G Studios" width={56} height={56} className="w-full h-full object-cover" />
            </div>
          </div>

          <h1 className="font-display font-bold text-2xl md:text-4xl text-text-primary tracking-tight">
            Richard G Studios
          </h1>
          <p className="text-text-secondary mt-2 md:mt-3 text-sm md:text-base leading-relaxed max-w-[240px] md:max-w-xs mx-auto">
            Seu ambiente de trabalho com IA
          </p>
        </motion.section>

        {/* ── Cards ── */}
        <section className="flex flex-col gap-2 md:gap-4 shrink-0 w-full">
